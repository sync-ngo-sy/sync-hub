...
`select` may derive a _view_ from already-canonical cached data (e.g. picking one field, sorting a
...
- No generic `isRecord`/`asRecord`/coercion toolkit and no manual probing of unknown properties. The boundary owner defines the exact schema it expects and parses once.
- No speculative shared helper. Keep an operation local until two current consumers need the same stable contract and Zod or the selected library does not already own it.
