...
quality_items as materialized (
  select
    ...
    case
      ...
      when summary_items.parsed_percentage < 75 or summary_items.extraction_confidence < 65 then 'review'
      else 'healthy'
    end as quality_band,
    (
      summary_items.parsed_percentage < 75
...
grant execute on function public.parsing_overview_page_v1(uuid[], integer, integer, boolean) to authenticated;
