...
export const protectedRoutes: RegisteredRoute[] = [
  ...
  {
    path: "compare",
    element: <IntelligentComparisonPage/>,
  ...
  },
  {
    ...
    subtitle: "Invite users, create tenants, and manage workspace access",
  },
  {
    path: "admin/settings",
    element: <PlatformRuntimeSettingsPage/>,
    title: "Runtime settings",
    subtitle: "Operational settings for extraction, retrieval, and model behavior",
  },
  {
    path: "admin/alerts",
    ...
    element: (
      <div className="page-stack">
        <EmptyState
          ...
          detail="This screen has not been mapped into the frontend router yet."
        />
      </div>
    ),
  },
...
export const adminNavigation: NavigationItem[] = [
  {
    ...
    match: (path) => path === "/admin" || path === "/admin/dashboard",
  },
  {
    to: "/admin/accounts",
    label: "Account provisioning",
    icon: UserPlus,
    match: (path) => path === "/admin/accounts",
  },
  {
    to: "/admin/settings",
    label: "Runtime settings",
    icon: Settings2,
    match: (path) => path === "/admin/settings",
  },
  {
    to: "/admin/alerts",
    label: "Alerts",
    icon: Bell,
    match: (path) => path === "/admin/alerts",
  },
  {
    to: "/admin/manatal-sync",
    label: "Manatal Sync",
    icon: RefreshCw,
    match: (path) => path === "/admin/manatal-sync",
  },
  {
    to: "/admin/search-simulator",
    label: "Search Simulator",
    icon: SlidersHorizontal,
    match: (path) => path === "/admin/search-simulator",
  },
  {
    to: "/admin/parsing",
    ...
    match: (path) =>
      path === "/admin/parsing" ||
      (path.startsWith("/admin/parsing/") && path !== "/admin/parsing/lab"),
  },
  {
    to: "/admin/parsing/lab",
    label: "Parsing Lab",
    icon: FlaskConical,
    match: (path) => path === "/admin/parsing/lab",
  },
];

export function routeChromeForPath(path: string) {
  return (
    protectedRoutes.find((route) => route.title && matchRoutePattern(route.path, path)) ??
    protectedRoutes[0]
  );
}
