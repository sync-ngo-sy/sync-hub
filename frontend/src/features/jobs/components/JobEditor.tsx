// frontend/src/features/jobs/components/JobEditor.tsx
...
export function JobEditor({initialForm, onSaved}: JobEditorProps) {
  ...
  return (
    <Panel className="job-editor-panel" style={{ width: "100%", maxWidth: "100%", margin: "0", boxSizing: "border-box" }}>
      <form className="job-editor" onSubmit={handleSubmit} style={{ width: "100%", maxWidth: "100%" }}>
        {/* Removed Title & Subtitle. Left only the Status Tag aligned to the right. */}
        <div className="job-editor__header" style={{ display: "flex", justifyContent: "flex-end", marginBottom: "1.5rem" }}>
        ...
        <div className="parser-form-grid">
          <label className="parser-field">
            ...
            <input className="form-input" value={form.title}
                   onChange={(event) => update("title", event.target.value)} required/>
          </label>
          <label className="parser-field">
            <span>Employer name</span>
            <input className="form-input" value={form.employerName}
                   onChange={(event) => update("employerName", event.target.value)} required/>
          </label>
          <label className="parser-field">
            <span>Employer country</span>
            <input className="form-input" value={form.employerCountry}
                   onChange={(event) => update("employerCountry", event.target.value)} required/>
          </label>
          <label className="parser-field">
            <span>Employer region</span>
            <select className="form-select" value={form.employerRegion}
                    onChange={(event) => update("employerRegion", event.target.value as EmployerRegion)}>
          ...
          <label className="parser-field parser-field--full">
            ...
            <textarea className="form-textarea job-editor__jd" value={form.jobDescription}
                      onChange={(event) => update("jobDescription", event.target.value)} required/>
          </label>
        </div>
        ...
        <div className="job-editor__extract-row">
          ...
          {extraction?.warnings.length ? <span
            className="job-editor__warning">{extraction.warnings.map((item) => item.message).join(" ")}</span> : null}
        </div>
        ...
        <div className="parser-form-grid">
          ...
          <label className="parser-field parser-field--full">
            ...
            <input className="form-input" value={form.preferredSkills}
                   ...
                   placeholder="Next.js, Banking domain"/>
          </label>
          <label className="parser-field">
            <span>Seniority level</span>
            <select className="form-select" value={form.seniorityLevel}
                    onChange={(event) => update("seniorityLevel", event.target.value)}>
          ...
          <label className="parser-field">
            ...
            <select className="form-select" value={form.remotePolicy}
          ...
          <label className="parser-field">
            ...
            <input className="form-input" value={form.locationCountry}
                   onChange={(event) => update("locationCountry", event.target.value)}/>
          </label>
          <label className="parser-field">
            <span>Job city</span>
            <input className="form-input" value={form.locationCity}
                   onChange={(event) => update("locationCity", event.target.value)}/>
          </label>
          <label className="parser-field parser-field--full">
            <span>Key responsibilities</span>
            <textarea className="form-textarea" value={form.keyResponsibilities}
                      onChange={(event) => update("keyResponsibilities", event.target.value)}/>
          </label>
        </div>
          ...
          {form.isPublic ? (
            <div className="parser-form-grid">
              <label className="parser-field">
                ...
                <input className="form-input" value={form.publicSlug}
                       onChange={(event) => update("publicSlug", event.target.value)}
                       placeholder="senior-react-developer-dubai"/>
              </label>
              <label className="parser-field">
                <span>Public title</span>
                <input className="form-input" value={form.publicTitle}
                       onChange={(event) => update("publicTitle", event.target.value)}
                       placeholder={form.title || "Public job title"}/>
              </label>
              <label className="parser-field">
                <span>Public location</span>
                <input className="form-input" value={form.publicLocation}
                       onChange={(event) => update("publicLocation", event.target.value)}
                       placeholder="Dubai, UAE"/>
              </label>
              <label className="parser-field">
                <span>Applications</span>
                <select className="form-select" value={form.publicApplyEnabled ? "open" : "closed"}
                        onChange={(event) => update("publicApplyEnabled", event.target.value === "open")}>
              ...
              <label className="parser-field parser-field--full">
                ...
                <input className="form-input" value={form.publicSummary}
              ...
              <label className="parser-field parser-field--full">
                ...
                <textarea className="form-textarea" value={form.publicDescription}
        ...
        <div className="job-editor__actions">
          <button className="button button--secondary" type="button" onClick={() => handleSave("draft")}
                  disabled={saveMutation.isPending}>
            <Save size={16}/>
            Save Draft
          </button>
          <button className="button button--primary" type="button" onClick={() => handleSave("active")}
                  disabled={saveMutation.isPending}>
            <CheckCircle2 size={16}/>
            Publish Active
          </button>
          {form.id ? (
            <button className="button button--secondary" type="button" onClick={() => handleSave("closed")}
                    disabled={saveMutation.isPending}>
              <XCircle size={16}/>
              Close
            </button>
